...
//GPU kernel 
__global__
void vecAdd(int *A,int *B,int *C,int saxpyNumber){
...
int main(int argc,char **argv)
{
    ...
    int *a_d,*b_d,*c_d;
    block_size = 400; //threads per block
   ...
   //GPU memory allocation
    ...
    vecAdd<<<block_no,block_size>>>(a_d, b_d, c_d, saxpyNumber);
    cudaCheckError();
    ...
    printf("n = %d \t GPU time = %fs \t CPU time = %fs during saxpy CUDA operation\n", n, time_d, time_h);
